...
export function SiteHeader() {
  ...
  return (
                ...
                <li key={item.to} className="shrink-0">
                  {item.external ? (
                    ...
                    <NavLink
                      ...
                      className={({ isActive }) =>
                        `${itemClass} ${
                          isActive ? "text-accent" : "hover:bg-muted/40 hover:text-accent"
                        }`
                      }
                    >
                      {item.label}
    ...
    {/* Mobile: right-side vertical popout drawer — rendered as sibling so it escapes the header's backdrop-filter containing block */}
    {open && (
      <nav
        id="primary-nav"
        aria-label="Primary"
